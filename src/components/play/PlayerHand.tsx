...
const PlayerHand: React.FC<PlayerHandProps> = ({ currentPlayerHand, useCard }) => {
  ...
  const getCardColor = (color: string) => {
    ...
    return colorMap[color] || "bg-gradient-to-br from-gray-500 to-gray-800";
  };
          ...
          <AnimatePresence>
            {currentPlayerHand.map((card, index) => (
              <motion.div
                ...
                className={`relative w-20 h-28 rounded-xl shadow-2xl flex flex-col items-center justify-center text-white text-xs text-center p-2 cursor-pointer border-2 border-white/30 overflow-hidden ${getCardColor(card.color)} card-in-hand`}
